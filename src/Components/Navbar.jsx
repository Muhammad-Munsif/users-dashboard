...
const Navbar = ({ toggleSidebar }) => { // <--- RECEIVES toggleSidebar prop
  const [searchTerm, setSearchTerm] = useState("");
  const [isProfileMenuOpen, setIsProfileMenuOpen] = useState(false);

  const toggleProfileMenu = () => setIsProfileMenuOpen(!isProfileMenuOpen);

  const user = { name: "John Profile" };
  ...
  return (
    <>
      <header className="w-full shadow-md bg-white border-b border-gray-100 sticky top-0 z-50 px-2 py-2 md:px-2 md:py-5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 space-x-4">
            
            {/* 1. Left Menu Icon - Calls the TOGGLE function */}
            <button
              className="p-2 text-indigo-700 hover:bg-gray-100 rounded-full transition duration-150"
              aria-label="Toggle Menu"
              onClick={toggleSidebar} // <--- ATTACHED FUNCTION
            >
              {/* Use Menu icon for the desktop toggle */}
            ...
            <div className="relative flex items-center space-x-4 ml-auto">
              {/* ... Notification Icons ... */}
              <NotificationIcon IconComponent={Bell} count={2} color="bg-red-500 absolute top-2 right-3" />
              <NotificationIcon IconComponent={MessageCircle} count={2} color="bg-red-500 absolute top-2 right-3" />
              ...
              <div className="relative flex-shrink-0">
                ...
                {isProfileMenuOpen && (
                  ...
                  >
                    ...
                    <a href="#" className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 transition duration-100 ease-in-out">
                      <Settings className="w-4 h-4 mr-3 text-indigo-500" /> Settings
                    </a>
